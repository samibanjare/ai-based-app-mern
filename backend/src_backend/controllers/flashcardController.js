...
export const reviewFlashcard = async(req,res, next)=>{
    try {
        ...
        res.status(200).json({
            success: true,
            data: flashcardSet,
            message: 'Flashcard reviwed successfully'
        })
    } catch (error) {
        next(error);
    }
...
export const toggleStarFlashcard = async(req, res, next)=>{
    try {
        ...
        //Toggle star
        flashcardSet.cards[cardIndex].isStared = !flashcardSet.cards[cardIndex].isStared;
        ...
        res.status(200).json({
            success: true,
            data: flashcardSet,
            message: `Flashcard ${flashcardSet.cards[cardIndex].isStared ?'stared':'unstared'}`
        });
